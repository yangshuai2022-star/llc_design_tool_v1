...
class LLCMainWindow(QMainWindow):
    ...
    def _build_actions(self) -> None:
        ...
        run_actions = [
            ("运行设计", self.run_design),
            ("快速波形", lambda: self.run_waveforms(False)),
            ("详细波形", lambda: self.run_waveforms(True)),
            ("小信号", lambda: self.run_small_signal({})),
            ("数字环路", lambda: self.run_digital_loop({})),
        ]
        for label, callback in file_actions:
        ...
        # Expanding spacer pushes the execute actions to the toolbar's right.
        spacer = QWidget()
        spacer.setSizePolicy(QSizePolicy.Policy.Expanding,
                             QSizePolicy.Policy.Preferred)
        toolbar.addWidget(spacer)
        for label, callback in run_actions:
            action = QAction(label, self)
            action.triggered.connect(callback)
            toolbar.addAction(action)
    ...
    def _build_parameter_panel(self) -> QWidget:
        ...
        actions = QGroupBox("运行")
        action_layout = QVBoxLayout(actions)
        self.run_button = QPushButton("运行 LLC 完整计算")
        self.fast_wave_button = QPushButton("显示快速关键波形")
        self.detail_wave_button = QPushButton("显示详细开关波形")
        self.small_signal_button = QPushButton("建立 G(s) / G(z)")
