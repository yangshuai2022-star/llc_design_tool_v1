...
- Vienna workspace now uses the same compact toolbar/diagram/hidden-parameter interaction model as TTPL/LLC; the oversized title row and 250–340 px diagram were removed.
- Added TTPL and Vienna **Inductor Design** pages. Default core is Magnetics High Flux Core Data 254 with editable geometry/AL and selectable supported permeability grades; default material grade is 60 µ.
- Added manufacturer-fit DC-bias inductance droop, full-load L(I) curve, line-cycle switching-ripple/core-loss calculation, enamelled round-copper sizing, hot DCR and DC I²R copper loss. Skin/proximity loss is intentionally disabled by default.
- Added one-click application of calculated full-load L and hot DCR back into TTPL/Vienna power-stage parameters.
...
## V7.2.1 — PFC UI density / TTPL diagram / Windows polish

- Reworked the TTPL PFC control overview to follow the LLC digital-control workspace: one compact control toolbar, one readable three-band vector diagram, then a horizontal parameter/result splitter.
- Reduced the TTPL overview scene from the old very wide signal chain to a compact voltage-loop / current-loop / sensing-feedforward layout; long physical sensor-return wires are represented semantically instead of crossing the whole schematic.
- Added `隐藏框图/显示框图` and `隐藏参数/显示参数` controls so Bode and waveform pages can reclaim vertical/horizontal space without losing the interactive overview.
- Moved TTPL Run / C99 Codegen into the same compact header as diagram zoom/fullscreen controls, removing the redundant large page-title row.
- Made Bode transfer-function checkboxes collapsible and collapsed by default; all per-transfer-function on/off controls are preserved.
- Moved Bode cursor and phase-budget status into one LLC-style horizontal status strip.
- Added a PFC workspace stylesheet aligned with LLC to normalize toolbar, tabs, group boxes, spin boxes, buttons, splitters and status bar on Windows 11/macOS/Linux.
- Updated the reusable vector block renderer so compact (<58 px) blocks keep title/subtitle text inside the block without clipping.
